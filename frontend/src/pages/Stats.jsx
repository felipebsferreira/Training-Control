...
const TABS = [
  { key: "body", label: "Composição corporal" },
  { key: "progress", label: "Progressão e recordes" },
...
export default function Stats() {
  ...
  const [weightHistory, setWeightHistory] = useState(undefined);

  const [progressLoaded, setProgressLoaded] = useState(false);
  const [workouts, setWorkouts] = useState(undefined);
  const [selectedExerciseId, setSelectedExerciseId] = useState(null);
  const [loadHistory, setLoadHistory] = useState(null);
  const [personalRecords, setPersonalRecords] = useState(undefined);
  const [volumeHistory, setVolumeHistory] = useState(undefined);

  const [frequencyLoaded, setFrequencyLoaded] = useState(false);
  const [consistency, setConsistency] = useState(undefined);
  const [workoutRanking, setWorkoutRanking] = useState(undefined);
  ...
  function openTab(tab) {
    ...
    if (tab === "body" && !bodyLoaded) {
      ...
      Promise.all([getProfile(), getWeightHistory()])
        ...
        .catch(() => setError("Não foi possível carregar a composição corporal"));
    }

    if (tab === "progress" && !progressLoaded) {
      setProgressLoaded(true);
      Promise.all([listWorkouts(), getPersonalRecords(), getVolumeHistory()])
        .then(([workoutsData, records, volume]) => {
        ...
        .catch(() => setError("Não foi possível carregar progressão e recordes"));
    }

    if (tab === "frequency" && !frequencyLoaded) {
      setFrequencyLoaded(true);
      Promise.all([getConsistency(), getWorkoutRanking()])
        .then(([consistencyData, ranking]) => {
          setConsistency(consistencyData);
          setWorkoutRanking(ranking);
        })
        .catch(() => setError("Não foi possível carregar as métricas de frequência"));
    }
  }
  ...
  if (!summary) return <p className="text-slate-500">Carregando...</p>;
  ...
  return (
    <div>
      ...
      {!activeTab && (
        <p className="text-slate-400 text-sm">Selecione uma categoria acima para ver os detalhes.</p>
      )}
