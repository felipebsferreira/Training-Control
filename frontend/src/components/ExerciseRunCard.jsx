...
export default function ExerciseRunCard({
  sessionExercise,
  logId,
  readOnly,
  completed,
  ...
  const loadDisplay = isPyramid
    ? `${loadSummary} ${sessionExercise.loadUnit}`
    : sessionExercise.load != null
    ? `${sessionExercise.load} ${sessionExercise.loadUnit}`
    : "—";

  // The pre-session (readOnly) view has no collapse concept — it always
  // shows the full summary. During an active session, content only shows
  // when this card is the one expanded (see TodayWorkout's accordion state).
  const showDetails = readOnly || expanded;
    ...
    >
      <div className="flex items-start justify-between gap-2">
        {readOnly ? (
          ...
          >
            <span className="text-slate-400 mt-1 shrink-0 text-xs">{expanded ? "▾" : "▸"}</span>
            {nameBlock}
          </button>
        )}
        {!readOnly && (
      ...
      {!showDetails ? (
        <p className="text-xs text-slate-500">
          {sessionExercise.sets.length} séries · {repsSummary} · {loadDisplay}
        </p>
      ) : (
        <>
          ...
          {readOnly ? (
            <div className="pt-2 border-t border-slate-100">
              <p className="text-sm text-slate-500">Carga: {loadDisplay}</p>
